...
namespace
{
    const char* const WINDOW_TITLE = "Ethan Final Project"; // Macro for window title
    ...
    glm::vec3 gObjectColor2(.5f, .5f, .5f);
...
const GLchar* lamp2FragmentShaderSource = GLSL(440,
    ...
    out vec4 fragmentColor2; // For outgoing lamp color (smaller cube) to the GPU

void main()
{
    fragmentColor2 = vec4(.5f); // Set color to white (1.0f,1.0f,1.0f) with alpha 1.0
}
);
...
int main(int argc, char* argv[])
{
    ...
    if (!UCreateShaderProgram(lamp2VertexShaderSource, lamp2FragmentShaderSource, gLamp2ProgramId))
        return EXIT_FAILURE;
...
void URender()
{
    ...
    GLint objectColorLoc2 = glGetUniformLocation(gLamp2ProgramId, "objectColor2");
